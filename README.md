...
The privacy audit identified direct identifiers and conditional sensitive behavioural fields as the primary residual exposure in the bias-remediated dataset. As remediation, four direct identifiers were pseudonymised: `ssn` was hashed with SHA-256, `email` with keyed HMAC-SHA-256, `full_name` was replaced with the application id as an opaque token, and `ip_address` was generalised to the /24 subnet. Three conditional sensitive spending fields (`spending_alcohol`, `spending_gambling`, `spending_adult_entertainment`) were removed because pseudonymisation does not reduce inference risk for numeric behavioural amounts. The remediated dataset is exported to `data/processed/remediated_credit_applications.parquet` (500 records). This dataset represents the final privacy-reduced analytical output of the three-notebook pipeline. Remaining attributes still constitute personal data in a credit decisioning context and require appropriate access control, retention enforcement, and audit logging.

*All analysis is documented and reproducible in `notebooks/03-privacy-demo.ipynb`.*

## 9. Recommendations

This section consolidates forward-looking recommendations across all three audit dimensions. Each recommendation is ordered by severity within its subsection. Remediation actions already applied to the dataset are documented in Sections 6.7, 7.6, and 8.8. The recommendations below address production-level controls that NovaCred should implement to prevent recurrence and sustain compliance.

NovaCred's credit decisioning system cannot be deployed in its current state. The bias audit confirmed discriminatory decisioning by gender that persists after all financial controls (OR = 1.98, p = 0.0004), which must be remediated before any further automated credit decisions are made. A Data Protection Impact Assessment under GDPR Art. 35 is required and must be completed before processing resumes. Six of seven EU AI Act high-risk obligations under Art. 9-15 are not evidenced, all of which are legal preconditions for deployment under the conformity assessment framework.
...
R1 - Enforce primary key uniqueness at ingestion (Critical). A database-level uniqueness constraint on id should be enforced, with duplicates routed to a quarantine queue for manual review (see Section 6.3). Success criterion: no duplicate IDs enter any processed dataset.

R2 - Validate income presence and type at ingestion (Critical). Income must be validated as present, numeric, and positive at ingestion. Records failing this check should be blocked from automated approval until reviewed (see Section 6.2). Success criterion: 100% of records have a valid canonical income value before any decision is made.

R3 - Enforce mandatory event timestamps (High). Timestamps should be non-nullable and generated automatically at ingestion (see Section 6.2). Success criterion: timestamp completeness reaches 100%.

R4 - Standardise date formats at ingestion (High). All dates should be converted to ISO 8601 at ingestion and stored as a parsed date type (see Section 6.3). Success criterion: zero parsing failures and consistent age derivations.

R5 - Standardise categorical encodings at ingestion (High). Controlled vocabulary mappings should be applied at intake for all categorical fields (see Section 6.3). Success criterion: no mixed encodings in processed datasets.

R6 - Enforce numeric domain constraints (High). Domain validation rules should be enforced at ingestion to block out-of-range values in fields such as debt_to_income and credit_history_months (see Section 6.4). Success criterion: zero out-of-domain values in processed datasets.

R7 - Validate contact field formats (Medium). Format validation should be applied at ingestion for contact fields (see Section 6.4). Success criterion: zero invalid formats in processed datasets.

R8 - Add automated data quality monitoring (Medium). Scheduled completeness and validity checks should be implemented across all critical fields, with alerting on threshold breaches.

### 9.2 Bias Mitigation Measures

R1 - Suspend automated approvals and conduct root-cause model audit (Critical). Gender predicts loan approval independently of all financial controls (see Section 7.2). NovaCred must place a governance hold on automated approvals, audit all model features for gender-correlated effects, and document the root cause and remediation plan. No new automated decisions should be finalised until the disparity is resolved. Regulatory basis: GDPR Art. 22, EU AI Act Annex III.

R2 - Remove ZIP code from all model inputs immediately (High). ZIP code is near-perfectly collinear with gender and violates data minimisation (see Section 7.4). Any geographic signal may only be reintroduced via a financially justified proxy after a privacy impact assessment. Regulatory basis: GDPR Art. 5(1)(c).

R3 - Remediate intersectional disparities for high-risk subgroups (High). Three gender x age subgroups show DI violations below 0.80, with female 26-35 at DI = 0.620 (see Section 7.4). NovaCred must implement disaggregated monitoring and treat each violating subgroup as a separate fairness incident.

R4 - DPO review of sensitive spending categories (High). Sensitive spending fields must be subject to immediate review under GDPR Art. 9 (see Section 7.4). These categories must be removed from any model feature set as a default position. Reinstatement requires a documented necessity assessment and proportionality review.

R5 - Implement disaggregated ongoing monitoring (High). DI ratio, DPD, and conditional OR for gender must be recomputed on every decision batch. Alert thresholds: DI below 0.85 triggers early warning, DI below 0.80 triggers mandatory review, conditional OR p below 0.05 triggers immediate escalation. Regulatory basis: EU AI Act Art. 9.

R6 - Investigate age-based financial risk correlation (Moderate). Age disparities are explained by financial controls but the mechanism requires documentation (see Section 7.3). NovaCred should evaluate alternative creditworthiness signals to avoid indirect age disadvantage.

### 9.3 Privacy Safeguards

R1 - Enforce privacy by default at the dataset layer (Critical). NovaCred must separate direct identifiers into an access-restricted identity store and provide a pseudonymised analytical dataset as the default artifact (see Section 8.2). Owner: Engineering and DPO. Done when: analytical datasets contain no raw direct identifiers and access to identity data is role-restricted with logging.

R2 - Replace opaque rejection reasons with a controlled taxonomy (Critical). NovaCred must implement a controlled reason-code taxonomy with applicant-facing explanations and a defined contestation path (see Section 8.5, Art. 22). Owner: Engineering and Compliance. Done when: each rejection has a specific reason code and contestation requests are tracked through a documented workflow.

R3 - Restrict conditional sensitive behavioural fields (Critical). NovaCred must justify the necessity of sensitive spending fields under Art. 5(1)(c). If not demonstrated, remove from routine analytics (see Section 8.2). Owner: DPO and Data Science. Done when: fields are removed or necessity is documented with restricted access.

R4 - Initiate a DPIA for automated credit decisioning (Critical). Required under GDPR Art. 35(3)(a). The DPIA must document necessity, proportionality, risk assessment, and mitigation measures. No further deployment until complete. Owner: DPO.

R5 - Implement complete decision audit logging (High). NovaCred must implement an append-only decision event log with non-nullable timestamps, model version identifiers, and reason codes (see Section 8.5, Art. 5(2)). Owner: Engineering. Done when: all decisions generate a complete audit record with tamper-evident retention.

R6 - Implement a human oversight process (High). NovaCred must define review criteria for contested decisions, implement a review queue, and log all review actions (see Section 8.6, Art. 14). Owner: Product and Compliance.

R7 - Implement consent and purpose management (High). NovaCred must implement consent versioning and withdrawal handling with purpose binding enforced (see Section 8.5, Art. 7). Owner: Compliance and DPO.

R8 - Adopt a retention schedule with automated deletion (High). NovaCred must adopt a retention schedule with automated deletion and deletion audit logs across all data stores (see Section 8.5, Art. 5(1)(e)). Owner: Compliance and Engineering.

R9 - Implement DSAR and deletion propagation (Medium). NovaCred must define an end-to-end DSAR workflow covering all data stores and record processing events for auditability (see Section 8.5, Art. 17). Owner: Compliance and Engineering.

R10 - Produce technical documentation for high-risk AI system (Medium). NovaCred must produce documentation covering intended use, training data provenance, performance metrics, and limitations (see Section 8.6). Owner: Data Science.

R11 - Establish ongoing fairness monitoring (Medium). NovaCred must establish periodic fairness checks with defined thresholds and escalation procedures integrated into release governance (see Section 7.5). Owner: Data Science.
...
All identified data quality issues were remediated programmatically across the three-notebook pipeline. Protected attributes and proxy variables were removed to prevent direct discriminatory feature access, (direct) identifiers were hashed and sensitive behavioural fields were removed to reduce re-identification exposure. These dataset-level remediations address the immediate analytical risks but do not resolve the underlying governance deficits. 
